...
const C = {
  ...
  purple:"#7c5af6", teal:"#14b8a6", text:"#e8edf5", muted:"#4a5568",
...
// ── 로컬 API 프록시를 통해 Make.com 호출 (CSP 우회)
...
const Icon = ({ name, size=16, color="currentColor" }) => {
  const p = {
    ...
    zap:<polygon points="13,2 3,14 12,14 11,22 21,10 12,10" fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round"/>,
    sparkle:<path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 17l-6.2 4.3 2.4-7.4L2 9.4h7.6z" fill="none" stroke={color} strokeWidth="1.8" strokeLinejoin="round"/>,
    check:<polyline points="20,6 9,17 4,12" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round"/>,
    download:<><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" fill="none" stroke={color} strokeWidth="2"/><polyline points="7,10 12,15 17,10" fill="none" stroke={color} strokeWidth="2"/><line x1="12" y1="15" x2="12" y2="3" stroke={color} strokeWidth="2"/></>,
    external:<><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" fill="none" stroke={color} strokeWidth="2"/><polyline points="15,3 21,3 21,9" fill="none" stroke={color} strokeWidth="2"/><line x1="10" y1="14" x2="21" y2="3" stroke={color} strokeWidth="2"/></>,
  };
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" style={{display:"block",flexShrink:0}}>{p[name]}</svg>;
};

// ── Make.com API 호출 (서버 프록시 경유)
...
function JobResultModal({ job, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div style={{background:"rgba(14,20,36,0.98)",border:`1px solid ${C.border}`,borderRadius:20,padding:32,maxWidth:640,width:"100%",maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}>
          ...
          <button onClick={onClose} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted,width:32,height:32,borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        {job.status === "processing" && (
          <div style={{textAlign:"center",padding:"32px 0"}}>
            <div style={{width:40,height:40,border:`2px solid ${C.border}`,borderTopColor:C.blue,borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px"}}/>
            <div style={{fontSize:14,color:C.mutedLight,...tx}}>Make.com 파이프라인 처리 중...</div>
            <div style={{fontSize:12,color:C.muted,marginTop:8,...tx}}>GPT → Higgsfield 영상 생성 → TTS → 자막 → Drive</div>
            <div style={{fontSize:11,color:C.muted,marginTop:4,...tx}}>약 3~5분 소요됩니다</div>
          </div>
        )}
        {job.status === "completed" && assets && (
          <div>
              ...
              {[{key:"video",label:"영상",ext:"MP4"},{key:"voice",label:"음성",ext:"MP3"},{key:"subtitle",label:"자막",ext:"SRT"}].map(({key,label,ext})=>(
                <div key={key} style={{background:"rgba(74,144,232,0.06)",border:`1px solid rgba(74,144,232,0.15)`,borderRadius:12,padding:16,textAlign:"center"}}>
                  ...
                  {assets[key]?.download_url ? (
                    <a href={assets[key].download_url} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4,background:C.grad,color:"#fff",padding:"6px",borderRadius:6,fontSize:10,fontWeight:600,textDecoration:"none"}}>
                      <Icon name="download" size={10} color="#fff"/>다운로드
                    </a>
                  ) : (assets[key]?.higgsfield_url ? (
                    <a href={assets[key].higgsfield_url} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4,background:C.grad,color:"#fff",padding:"6px",borderRadius:6,fontSize:10,fontWeight:600,textDecoration:"none"}}>
                      <Icon name="external" size={10} color="#fff"/>보기
                    </a>
            ...
            {assets.narration_script && (
              <div style={{background:"rgba(0,0,0,0.3)",borderRadius:10,padding:14}}>
                <div style={{fontSize:11,color:C.blue,fontWeight:700,marginBottom:6,...tx}}>나레이션 스크립트</div>
        ...
        {job.status === "error" && (
          <div style={{textAlign:"center",padding:"24px 0",color:"#f87171"}}>
            <div style={{fontSize:14,fontWeight:600,marginBottom:8,...tx}}>오류가 발생했습니다</div>
...
function MakeStudio({ onJobSubmit }) {
  ...
  const STEPS = [
    "요청 전송 중...", "Make.com 수신 확인...",
    "GPT-4o 스토리 생성...", "Higgsfield 영상 요청...",
    "파이프라인 실행 중..."
  ];
  ...
  const submit = async () => {
    ...
    try {
      ...
      setStatus("Make.com 파이프라인 실행 완료 — 영상 생성 중 (3~5분)");
    } catch (e) {
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:32}}>
      ...
      <div style={{marginBottom:14}}>
        <label style={{fontSize:11,color:C.muted,display:"block",marginBottom:5,...tx}}>Callback URL (선택 — 결과 받을 주소)</label>
        <input style={inp()} placeholder="https://webhook.site/your-url" value={form.callback_url} onChange={e=>set("callback_url",e.target.value)}/>
      </div>
      ...
      <button onClick={submit} disabled={loading||!form.project_name||!form.scene_description} style={{
        ...
        border:`1px solid ${loading?C.blue:"transparent"}`,color:"#fff",
        ...
        {loading ? (
          <><div style={{width:16,height:16,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>{status}</>
        ) : (
          <><Icon name="zap" size={16} color="#fff"/>MemoryLink 파이프라인 실행</>
        )}
      </button>

      {loading && (
        <div style={{marginTop:12}}>
          <div style={{background:"rgba(0,0,0,0.3)",borderRadius:8,height:4,overflow:"hidden"}}>
            <div style={{height:"100%",background:C.grad,borderRadius:8,animation:"progress 3s linear forwards"}}/>
          </div>
        </div>
      )}
    </div>
...
export default function Home() {
  ...
  return (
    <div style={{background:C.bg,color:C.text,minHeight:"100vh",...tx}}>
      ...
      {/* Hero */}
      <section style={{minHeight:"100vh",display:"flex",alignItems:"center",padding:"80px 6% 60px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{flex:1,maxWidth:520,position:"relative",zIndex:2,marginRight:40}}>
          ...
          <h1 style={{fontSize:"clamp(32px,4vw,56px)",fontWeight:800,lineHeight:1.1,letterSpacing:-1,marginBottom:20,...tx}}>
              ...
              {jobs.slice().reverse().map(job=>(
                <div key={job.id} onClick={()=>setSelectedJob(job)} style={{
                  ...
                  <div style={{
                    ...
                    background:job.status==="completed"?"rgba(34,197,94,0.15)":job.status==="error"?"rgba(248,113,113,0.15)":"rgba(74,144,232,0.15)",
                    color:job.status==="completed"?"#22c55e":job.status==="error"?"#f87171":C.blue,...tx
                  }}>{job.status==="completed"?"완료":job.status==="error"?"오류":"처리 중"}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        ::-webkit-scrollbar-thumb{background:rgba(74,144,232,0.25);border-radius:3px}
      `}</style>
    </div>
  );
}
